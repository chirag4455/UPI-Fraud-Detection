...
class UserBehaviorAnalyzer:
    ...
    def detect_behavioral_anomaly(self, user_id: str, current_txn: Dict, baseline: Dict) -> Tuple[float, List[str]]:
        ...
        if baseline.get('is_new_account'):
            ...
            if z_score > 3:
                score += 90
                reasons.append(f"CRITICAL: Amount {txn_amt:,.0f} is {z_score:.1f}σ above average ({avg_amt:,.0f})")
            elif z_score > 2:
                score += 70
                reasons.append(f"HIGH: Amount {txn_amt:,.0f} is {z_score:.1f}σ above average")
            elif z_score > 1:
                score += 40
...
class VelocityEngine:
    ...
    def _get_txns_in_window(self, user_id: str, minutes: int) -> List[Dict]:
        ...
        return [t for t in self.txn_history.get(user_id, []) if t['timestamp'] >= cutoff]
...
class PayeeValidator:
    ...
    def validate_payee(self, receiver_upi: str, receiver_name: str = None) -> Tuple[float, List[str]]:
        ...
        if not self._is_known_payee(receiver_upi):
            score += 65
            reasons.append("Payee is new/unknown - elevated risk")
        return min(score, 100), reasons
...
class CompromiseDetector:
    ...
    def check_compromise_signs(self, user_id: str, current_session: Dict) -> Tuple[float, List[str]]:
        ...
        if self._is_new_device(user_id, current_session.get('device_id')):
            score += 60
            reasons.append("WARNING: New/unknown device detected")
        return min(score, 100), reasons
    ...
    def detect_fraud(self, transaction_data: Dict) -> Dict:
        ...
        if payee_score >= 95:
            return {
                ...
                'layers': {
                    'payee': {'score': 100, 'weight': 1.0, 'reasons': payee_reasons}
                },
        ...
        layers['behavioral'] = {'score': behavioral_score, 'weight': 0.38, 'reasons': behavioral_reasons}
        total_score += behavioral_score * 0.38

        # Layer 2: Velocity - 0.04
        velocity_score, velocity_reasons = self.velocity_engine.check_velocity(
            user_id, validated_txn['amount']
        )
        layers['velocity'] = {'score': velocity_score, 'weight': 0.04, 'reasons': velocity_reasons}
        total_score += velocity_score * 0.04

        # Layer 3: Flow - 0.03
        flow_score, flow_reasons = self.flow_analyzer.analyze_flow(
            validated_txn['sender_upi'], validated_txn['receiver_upi'], validated_txn['amount']
        )
        layers['flow'] = {'score': flow_score, 'weight': 0.03, 'reasons': flow_reasons}
        total_score += flow_score * 0.03

        # Layer 4: Payee - 0.28
        layers['payee'] = {'score': payee_score, 'weight': 0.28, 'reasons': payee_reasons}
        total_score += payee_score * 0.28

        # Layer 5: Compromise - 0.15
        compromise_score, compromise_reasons = self.compromise_detector.check_compromise_signs(
            user_id, validated_txn
        )
        layers['compromise'] = {'score': compromise_score, 'weight': 0.15, 'reasons': compromise_reasons}
        total_score += compromise_score * 0.15

        # Layer 6: Amount - 0.10
        amount_score = self._basic_amount_check(validated_txn['amount'])
        amount_reasons = [] if amount_score == 0 else [f"High amount: INR {validated_txn['amount']:,.0f}"]
        layers['amount'] = {'score': amount_score, 'weight': 0.10, 'reasons': amount_reasons}
        total_score += amount_score * 0.10

        # Layer 7: Website - 0.02
        website_score = 0
        website_reasons = []
        ...
        layers['website'] = {'score': website_score, 'weight': 0.02, 'reasons': website_reasons}
        total_score += website_score * 0.02

        # Record txn in velocity engine
        self.velocity_engine.record_transaction(user_id, validated_txn['amount'], validated_txn['receiver_upi'])
        ...
        if final_score >= 80:
            ...
            verdict = 'SUSPICIOUS'
            status = 'REQUIRES_2FA'
            action = 'REQUIRES 2FA - Review transaction'
        elif final_score >= 40:
            verdict = 'CAUTION'
            status = 'APPROVED_WITH_WARNING'
            action = 'APPROVED (with warning) - Monitor account'
        else:
            verdict = 'SAFE'
            status = 'APPROVED'
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("TEST 1: Known scam UPI (should BLOCK, score 100)")
    ...
    print(f"  Score: {result['risk_score']} | Verdict: {result['verdict']} | Status: {result['status']}")

    print("\nTEST 2: Suspicious txn (should be ~69, REQUIRES_2FA)")
    result = detector.detect_fraud({
        'user_id': 'user_001',
        'sender_upi': 'john@okaxis',
    ...
    for layer, data in result['layers'].items():
        print(f"    {layer}: raw={data['score']} x {data['weight']} = {data['score']*data['weight']:.1f}")

    print("\nTEST 3: Normal txn (should be SAFE, <40)")
    result = detector.detect_fraud({
        'user_id': 'user_001',
        'sender_upi': 'john@okaxis',
    ...
    print("\nTEST 4: Medium risk - known payee but huge amount at night")
    result = detector.detect_fraud({
        'user_id': 'user_001',
        'sender_upi': 'john@okaxis',
        'receiver_upi': 'mom@ybl',
