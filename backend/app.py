...
import warnings
import uuid
from user_manager import register_user, verify_pin, get_user_balance, update_balance
from database import Transaction, User, WebsiteReputation, UserBehaviorBaseline
...
def predict_fraud(feature_df):
    ...
    # Amount scoring - MORE AGGRESSIVE
    if amount > 500000:  # 5 lakhs+
        reasons.append("EXTREMELY high amount (INR {:,.0f})".format(amount))
        shap_reasons.append({"feature": "Amount", "impact": 0.50, "width": 50})
        risk_score += 40
        ...
        shap_reasons.append({"feature": "Amount", "impact": 0.25, "width": 25})
        risk_score += 25
    else:
        shap_reasons.append({"feature": "Amount", "impact": -0.1, "width": 10})

    # Hour scoring - MORE AGGRESSIVE
    if hour >= 23 or hour <= 5:
        reasons.append("Very unusual hour ({}:00 - HIGH RISK PERIOD)".format(hour))
        shap_reasons.append({"feature": "Hour (Night)", "impact": 0.40, "width": 40})
        risk_score += 40
    elif hour >= 22 or hour <= 6:
        reasons.append("Unusual hour ({}:00)".format(hour))
        shap_reasons.append({"feature": "Hour", "impact": 0.25, "width": 25})
        risk_score += 20
    else:
        shap_reasons.append({"feature": "Hour", "impact": -0.05, "width": 5})

    # New Payee scoring - MORE AGGRESSIVE
    if feature_df["is_new_payee"].values[0] == 1:
        reasons.append("NEW PAYEE - First transaction EVER")
        shap_reasons.append({"feature": "New Payee", "impact": 0.35, "width": 35})
        risk_score += 30

    # Device scoring
    if feature_df["is_known_device"].values[0] == 0:
        reasons.append("Unknown/new device - RISKY")
        shap_reasons.append({"feature": "Unknown Device", "impact": 0.32, "width": 32})
        risk_score += 32

    # Balance drain
    if feature_df["balance_change_ratio"].values[0] > 0.5:
        reasons.append("Large balance drain (>50% of balance)")
        shap_reasons.append({"feature": "Balance Drain", "impact": 0.28, "width": 28})
        risk_score += 28

    # Transaction type
    if feature_df["is_transfer"].values[0] == 1 or feature_df["is_cash_out"].values[0] == 1:
        shap_reasons.append({"feature": "Txn Type (Transfer/CashOut)", "impact": 0.20, "width": 20})
        risk_score += 15
    ...
    return {
        "prediction": 1 if risk_score >= 70 else 0,
...
def predict():
    ...
    if request.method == "POST":
        ...
        alert = {
            ...
            "level": "critical" if result["risk_score"] >= 70 else "warning" if result["risk_score"] >= 45 else "safe"
        }
...
@app.route("/network")
def network_page():
    network = {"nodes": len(predictions_store)*2, "edges": len(predictions_store), "rings": sum(1 for p in predictions_store if p.get("risk_score",0)>70), "mules": sum(1 for p in predictions_store if 45<p.get("risk_score",0)<=70), "graph_html": None, "fraud_rings": []}
...
def api_login():
    ...
    try:
        ...
        db = _db.SessionLocal()
        user = db.query(User).filter(User.fake_upi_id == upi_id).first()
        db.close()
        
        if not user or not verify_pin(user.id, pin):
            return jsonify({'error': 'Invalid credentials'}), 401
        
        return jsonify({
            'user_id': user.id,
            'fake_upi_id': user.fake_upi_id,
...
def api_transfer():
    """Process money transfer with fraud detection"""
    try:
        data = request.json
        ...
        # Check receiver
        receiver = db.query(User).filter(User.fake_upi_id == receiver_upi).first()
        if not receiver:
            db.close()
            return jsonify({'error': 'Invalid receiver UPI'}), 400
        
        # Get website reputation
        domain = urlparse(website_url).netloc if website_url else 'unknown'
        website_rep = db.query(WebsiteReputation).filter(
            WebsiteReputation.domain == domain
        ).first() if website_url else None
        
        trust_score = website_rep.trust_score if website_rep else 50
        ...
        fraud_result = predict_fraud(feature_df)
        
        # Create transaction record
        transaction = Transaction(
            id=str(uuid.uuid4()),
            sender_upi=sender.fake_upi_id,
            receiver_upi=receiver_upi,
            amount=amount,
            website_url=website_url,
            website_trust_score=trust_score,
            fraud_flag=fraud_result['risk_score'] >= 70,
            models_verdict=json.dumps(fraud_result),
            status='BLOCKED' if fraud_result['risk_score'] >= 70 else 'COMPLETED'
        )
        ...
        response = {
            ...
            'fraud_detected': fraud_result['risk_score'] >= 70,
            'fraud_score': fraud_result['risk_score'],
            'reason': fraud_result.get('explanation', ''),
...
@app.route('/api/website-check/<domain>', methods=['GET'])
def api_check_website(domain):
    """Check website reputation"""
    try:
        db = _db.SessionLocal()
        website = db.query(WebsiteReputation).filter(
            WebsiteReputation.domain == domain
        ).first()
        db.close()
